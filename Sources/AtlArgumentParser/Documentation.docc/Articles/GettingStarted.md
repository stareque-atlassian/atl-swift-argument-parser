# Getting Started with AtlArgumentParser

Learn to set up and customize a simple command-line tool.

## Overview

This guide walks through building an example command. You'll learn about the different tools that `AtlArgumentParser` provides for defining a command's options, customizing the interface, and providing help text for your user.

## Adding AtlArgumentParser as a Dependency
...
and then include `"AtlArgumentParser"` as a dependency for our executable target.
...
let package = Package(
    ...
    targets: [
        .executableTarget(
            name: "count",
            dependencies: [.product(name: "AtlArgumentParser", package: "swift-argument-parser")]),
    ]
)
```
...
In the code above, the `inputFile` and `outputFile` properties use the `@Argument` property wrapper. `AtlArgumentParser` uses this wrapper to denote a positional command-line input — because `inputFile` is specified first in the `Count` type, it's the first value read from the command line, and `outputFile` is the second.
...
Finally, the `Count` command is designated as the program's entry point by applying the `@main` attribute. When running your command, the `AtlArgumentParser` library parses the command-line arguments, verifies that they match up with what we've defined in `Count`, and either calls the `run()` method or exits with a helpful message.
...
`AtlArgumentParser` automatically generates help for any command when a user provides the `-h` or `--help` flags:
